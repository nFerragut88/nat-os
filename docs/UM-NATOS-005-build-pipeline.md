...
This report documents how nat-os source becomes a bootable image, every
...
ends by calling user code. nat-os replaces all of that. The framework is
...
| Flag | Purpose |
|---|---|
| `-mabi=call0` | Select the non-windowed ABI. **The load-bearing flag** — see UM-NATOS-003 |
...
| `-T kernel/linker.ld` | Our memory map (UM-NATOS-004) |
| `-Wl,--gc-sections` | Discard unreferenced sections |
| `-Wl,-Map,build/natos.map` | Emit a map file — the authoritative record of what landed where |
...
UM-NATOS-002 identifies as the highest-value diagnostic when a boot fails.
