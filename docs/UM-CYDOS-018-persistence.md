# UM-CYDOS-018 — Persistence, and a Read Defect That Looked Like the Wrong Thing
...
cyd-os now has state that outlives a power cycle. A SPI flash driver writes a
...
This also closes the cache-off hazard flagged in UM-CYDOS-011 §4 — by a
...
UM-CYDOS-011 §4 identified this as the thing that would break first:
...
does. cyd-os does not. It makes the dangerous reads impossible instead:
...
marker in UM-CYDOS-016 and the boot banner in UM-CYDOS-017 §5.
...
- **No application access.** Persistence is kernel-only. No syscall exposes it,
  ...
  UM-CYDOS-016/017 would all need extending before one could.
...
- UM-CYDOS-011 §4 — the cache-off hazard predicted there, closed in §3 here
- UM-CYDOS-011 §2.2 — `.rodata` placement, extended to `flash.o` and `store.o`
- UM-CYDOS-016 §3 — the frozen marker block, first instance of the §5.1 pattern
- UM-CYDOS-017 §5 — the self-erasing capture, second instance
- UM-CYDOS-015 §5 — the SPI2 user-mode path this driver's framing borrows from
