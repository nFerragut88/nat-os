...
finished file. **390 pages**; spine width 0.9153 in on premium colour (0.8783 in
if the interior is printed on white). Rebuilt 2026-08-18 after reports 029–032
were folded in — it was 372 pages before that.
...
in one flat file, **13.165 × 9.250 in** with 0.125 in bleed. The 2 × 1.2 in
...
the page count, so a cover built against a 372-page interior does not fit a
390-page one — and that mismatch is the one error in this pipeline that a
proof-read cannot catch, because both files are individually valid.

The cover also *prints* figures that go stale: the page count, the report count
and the standing-rule count all appear as cover art. The page count is now read
from the measured interior; the other two are literals in `build_cover.py` and
have to be changed by hand. They were wrong until 2026-08-18 — the back cover
said 372 pages and twenty-eight reports on a 390-page book drawing on
thirty-two.
...
premium colour, which on 390 pages is the difference between a 13.128 in wrap
and a 13.165 in one — and submitting the wrong one is rejected with *"your
