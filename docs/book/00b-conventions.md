...
The CPU runs at 80 MHz. This is **derived**, not specified: UM-NATOS-008 §5.2
counted 25 ticks of 2,400,000 cycles within a ~10 s capture window and inferred
a clock near 80 MHz. The bootloader leaves the core at its default and nothing
in the kernel raises it, so this is a measurement of the current configuration
rather than a specification figure, and it should be re-derived if boot
