# UM-CYDOS-004 — Memory Map and Allocation Policy
...
This report records the ESP32 address regions relevant to cyd-os, the layout
...
> addresses cyd-os actually uses (§3) are confirmed working on hardware; the
...
| VM application arenas | DRAM, fixed-size blocks | **Bounds-checked by the interpreter** — the isolation mechanism (UM-CYDOS-001 §4.2) |
...
The measured boot trace (UM-CYDOS-002 §4) shows the second-stage bootloader
...
cyd-os declares as its IRAM** (`0x40080000`, length `0x20000`).
...
- UM-CYDOS-002 §4 — measured boot trace containing the overlapping load address
- UM-CYDOS-006 — hardware confirmation of the addresses in §3.1
