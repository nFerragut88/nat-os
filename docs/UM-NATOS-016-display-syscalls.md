# UM-NATOS-016 — Display Syscalls, and a Total System Freeze
...
This is deliberately the arena model applied to a second resource. UM-NATOS-013
...
exactly the reason `arena_contains()` does (UM-NATOS-010 §5.2). Coordinates
...
UM-NATOS-012 §5 would be worthless.
...
`_handler_level3`'s `PS.EXCM` rule (UM-NATOS-009 §6) is the other standing rule
