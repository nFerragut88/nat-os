# UM-CYDOS-007 — Development Roadmap M1–M5
...
**Prerequisite — resolve UM-CYDOS-004 §5.** M1 will push `.text` past the point
...
kernel, stacks, and eventually a display buffer (UM-CYDOS-004 §4 notes a full
...
- Dispatch loop with a per-instruction quantum counter — the mechanism enabling
  preemption at safe boundaries (UM-CYDOS-001 §4.2).
...
one's flash step. Copying both into `cyd-os/boot/` removes the coupling — about
21 KB.

## 9. References

- UM-CYDOS-001 — architecture and the isolation model driving M4/M5
- UM-CYDOS-003 §6 — why M2 is tractable
- UM-CYDOS-004 §5 — the risk blocking M1
- UM-CYDOS-006 — what M0 established
