# UM-NATOS-011 — Flash Cache and Read-Only Data Placement
...
in UM-NATOS-010 §7.3 and deferred since UM-NATOS-004 §3.
...
read-only data mapped, and nat-os inherits it by producing an image of the same
shape.
...
- **Cache-off windows become hazardous.** Any future code that writes flash must
  ...
  nat-os has no equivalent yet and does not need one until it writes flash, but
...
- **Cache configuration is now load-bearing.** The kernel depends on the
  ...
  code (UM-NATOS-001 §3), cache and MMU setup becomes our responsibility, and
