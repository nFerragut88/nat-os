...
UM-NATOS-006 and UM-NATOS-008 recorded the watchdog state as "inference, not
...
- **No memory protection.** The ESP32 has no MMU paging. A native task can
  ...
  Isolation arrives only with the bytecode VM (UM-NATOS-001 §4.2).
...
**`TASK_BLOCKED`** arrived with the mutex (UM-NATOS-014 §3). A blocked task is
...
same shape as §6 and as UM-NATOS-017 §6.
...
- UM-NATOS-003 — `call0` ABI selection; why register windows are absent
