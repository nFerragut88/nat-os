# UM-NATOS-010 — Milestone 3 Verification Report
...
applications will run inside. Its exit criteria (UM-NATOS-007 §5) are narrow and
...
On a kernel with no memory protection (UM-NATOS-001 §4.2) this is not
...
(UM-NATOS-004). Memory-mapped flash has no such restriction, so the constraint
that forced the current placement does not apply. Realising this requires
enabling the flash cache — the work deferred in UM-NATOS-004 §3 — and is
therefore a prerequisite worth scheduling before the asset-heavy milestones
rather than during them.

> **DONE — see UM-NATOS-011.** The `.rodata` row of this table is now fact
