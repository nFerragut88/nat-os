...
> **Since written — boot configuration changed, exactly as the warning
> anticipated, and the derivation was the thing that could not catch it.** A
> replacement second-stage bootloader left the SoC on the bare 40 MHz crystal
> (Ch. 1 §1.2). The tick is programmed in *cycles*, so it kept firing every
> 800,000 of them; what changed was that 800,000 cycles had become 20 ms of wall
> clock rather than 10. Every counter stayed self-consistent, every self-test
> passed, and `fullscreen=44 ms` printed the same 44.
>
> A derived figure is only as good as the derivation's independent reference,
> and this one's — counting ticks against the host clock over a capture window —
> is the one that would have caught it. It was not re-run, because nobody thought
> the clock was a variable. `kernel/clock.c` now sets the frequency instead of
> inheriting it, `clock_cpu_mhz()` reports it, and UM-NATOS-036 §11 re-derived
> the rate against an independent clock rather than against `CCOUNT`.
