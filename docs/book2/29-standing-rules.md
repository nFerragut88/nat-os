...
> can only return one answer. The touch X axis was backwards for three days
...
Seven more, from the two sessions that closed the one-bit display defect
(Chapters 28b and 28c): six from UM-NATOS-030 and one from UM-NATOS-029. They
are kept separate rather than renumbered into §29.2–29.4 because the numbered
twelve are cited by number elsewhere in the record.
...
### Rule 19 — A guard can only be shown unnecessary by measuring it in its absence

The display's DMA wait had a bound of 2,000,000 cycles that tripped spuriously.
The bound was raised to 40,000,000; `dmastat` was then read as `timeouts=0`, and
that was taken as evidence the raise had been unnecessary. **The raise is why it
read zero.** The change was reverted on that reasoning, and the defect stayed in
the tree for several more hours until the original bound was back in place long
enough to read `1`.

The general form is worth stating carefully, because the reasoning is seductive
in a way that Rules 5 to 9 are not: those catch instruments that are broken, and
this catches an instrument that is working perfectly and being asked a question
it cannot answer. A guard that never fires is indistinguishable, from inside,
between *never needed* and *needed and doing its job*. Only removing it — or
finding the record from before it existed — separates the two.

It arrived a second time in the same week, from the other direction: the same
guard, having been shown necessary, turned out to be **load-bearing for a
defect** (Rule 17). One bound, two rules, and both of them are about the fact
that a system which has run for a while is not a set of independent parts.

**Origin:** UM-NATOS-029 §7.2, restated in UM-NATOS-030 §5.4. Chapters 28b
§28b.7.2 and 28c §28c.5.
...
24. If a guard is being removed as unnecessary, was it measured in its absence?
