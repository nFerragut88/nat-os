...
> Sources: the "what this does not establish" section of all thirty reports
> Code: throughout
...
Four predictions came true, and the fourth was added to this table after the book
was first compiled — by the gap it named coming due:
...
| UM-NATOS-008 §5.2 | The CPU clock is derived, not specified, and should be re-derived if boot configuration changes | §30.6 — boot configuration changed and the derivation was not re-run. The board ran at half speed and every instrument agreed it had not |
...
> the FIFO path, where the bug genuinely was not present. Chapter 28 §28.11, and
> Chapters 28b and 28c for the two sessions in full.

### What the closure did not close

Worth stating separately, because a closed fault is where an inventory of gaps
usually stops reading. Three questions from that investigation are answered and
three are not:

**Answered.** Why the view garbled; why it healed on a timer; why two programs
appeared to repair it and a third never did; why raising a guard made the device
worse.

**Not answered, and none of them has been retested since the fix:**

- **Whether the low-uptime symptom was ever two faults.** The record contains one
  observation that at very low uptime the view *did not open at all*, rather than
  opening wrongly. That was never reproduced under instrumentation, and "does not
  open" and "opens garbled" are different failures.
- **Whether any of the eight instruments built during the failed session still
  earn their place.** They were built to discriminate between theories that are
  now all known to be downstream. `dmastat` and `fbdump` clearly do; `resyncn`
  and `stripn` exist to answer a question nobody will ask again.
- **What the DMA fix changed elsewhere.** Every measurement in the display and
  renderer chapters was taken on the FIFO fallback (App. F §F.10). Only the blit
  has been remeasured. The frame budget, the bus share, the framebuffer's value
  and the touch-versus-render contention were all measured on a machine that no
  longer exists.
...
- ~~**The CPU frequency is derived, not read.**~~ **Closed, by way of being
  right.** The entry read: 80 MHz, inferred from tick counts against a host
  clock, and *"it should be re-derived rather than assumed if boot configuration
  changes"*. Boot configuration changed — a replacement second-stage bootloader
  that does not touch the clock — and the board ran at 40 MHz with **no
  instrument in the system able to see it**, because all of them derive from
  `CCOUNT`. `kernel/clock.c` now sets the frequency and reports it, so it is read
  rather than inferred (Ch. 1 §1.2, Ch. 7 §7.8). This is the fourth prediction in
  §30.1's table, and the only one where the gap and its consequence were named in
  the same sentence.
