...
| L1 | Second-stage bootloader: clock config, flash cache/MMU init, image loading | **Borrowed.** Replaceable later — *and since replaced; see the note below.* |
...
> **Since written — L1 is no longer borrowed, and item 3 was the bill for it.**
>
> `boot/` now holds this project's own second-stage bootloader: 2,736 bytes when
> UM-NATOS-035 measured it, against Espressif's 17,536, and `build.ps1 -Flash`
> builds and flashes it by default. It does three memory copies and one
> address-space mapping, reusing `kernel/flash.c` unmodified — a driver written
> for a persistent store, which worked inside a program that runs before the
> kernel exists because it never depended on the kernel existing.
>
> Espressif's binary stays in `vendor/` as the recovery image and the A/B
> control (`build.ps1 -Flash -VendorBootloader`), which is the whole reason it
> was ever kept.
>
> **Item 3 is what that cost.** Espressif's loader switches the SoC to the
> 320 MHz PLL and divides it to 80 MHz before jumping. nat-os had inherited that
> for its entire life with no part of the kernel knowing it depended on it. Ours
> copies segments and jumps, so the board came up on the bare 40 MHz crystal —
> **half speed, and every instrument in the system was blind to it**, because
> every duration this kernel reports is derived from `CCOUNT`. Halve the clock
> and a duration measured in cycles prints the same number for twice the
> wall-clock time. `display init ok fullscreen=44 ms` was identical before and
> after, and it was 44 ms of cycles against 88 ms of reality.
>
> `kernel/clock.c` now owns the clock, and owns it in the *kernel* rather than
> in the loader on purpose: choosing a CPU frequency is an operating system's
> decision, and putting it here makes nat-os independent of what loaded it —
> which is precisely the property whose absence caused the regression.
> `clock_init()` returns immediately if it finds the SoC already on the PLL, so
> one image boots correctly from either loader.
>
> So the layer table now reads: **L0 silicon, L1 ours, L2–L4 ours.** The
> executable chain from reset to shell prompt is this project's own code except
> for the part that is physically in the chip. UM-NATOS-035 and UM-NATOS-036
> are the full account, including the one consequence still open: with our
> loader the WiFi PHY fails differently and `-WiFi` builds are gated to
> Espressif's second stage until that is understood.
