...
## A.2b The second-stage bootloader

Added after this book was compiled (UM-NATOS-035, Ch. 3 §3.1.1). It is a
separate program with its own link map, built and flashed by `build.ps1 -Flash`,
and it compiles three kernel files again rather than linking against the kernel —
which is not in memory yet.

| File | Contents | Chapter |
|---|---|---|
| `boot/boot_start.S` | Entry from the ROM: a stack of its own, `.bss` zeroed (it is `NOLOAD`, so on entry it holds whatever the last boot left), `call0 boot_main` | 3 |
| `boot/boot.c` | The work: watchdog off, header parse at `0x10000`, flash MMU mapping, segment copies, cache enable, jump | 3 |
| `boot/boot.ld` | IRAM `0x40078000`, DRAM `0x3FFF0000` — both clear of where nat-os lands, which is the constraint that decides both. No DROM region: this runs with the cache off | 3, 4 |
| `boot/build_boot.ps1` | Builds and optionally flashes; recompiles `kernel/flash.c`, `kernel/uart.c`, `kernel/kstring.c` | 3, 5 |
| `boot/README.md` | What the job is, why it was achievable, and the recovery command | 3 |

`kernel/clock.c` belongs with them in spirit and in the kernel in fact: it
switches the SoC to the 320 MHz PLL and divides it to 80 MHz, because the
loader deliberately does not (Ch. 1 §1.2).

| File | Contents | Chapter |
|---|---|---|
| `kernel/clock.c` | PLL bring-up over the ROM's `rom_i2c_writeReg`, the crystal check that refuses an unknown part, `uart_rescale()`, `clock_cpu_mhz()` | 1, 7 |

## A.3 Vendor

| Path | Bytes | Contents | Chapter |
|---|---|---|---|
| `vendor/bootloader.bin` | 17,536 | ESP-IDF second-stage bootloader, Apache 2.0. **No longer in the boot path** — the recovery image and the A/B control (`-VendorBootloader`) | 3 |
| `vendor/partitions.bin` | 3,072 | Partition table, Apache 2.0. Still flashed at `0x8000`, **read by nothing**: `boot.c` hardcodes the image offset | 3 |
...
| `LICENSE` | MIT | — |
| `docs/README.md` | Report index, status table, the standing rules | 29 |
| `docs/UM-NATOS-0NN-*.md` | The engineering reports; this book synthesises 001–030 | Appendix E |
| `docs/pdf/` | Rendered PDFs of every report | — |
| `docs/style/` | `build_pdfs.py`, `figures.py`, `report.css`, and the book and cover builders | — |
| `docs/book2/` | This book | — |
| `boot/` | The second-stage bootloader — §A.2b | 3 |
| `tools/serial/grab_fb.py` | Host half of `fbdump`: reads to `FBEND`, refuses an incomplete capture, writes a PNG | 28c |
