...
> **Since written — the transfer also resets the engine, and which resets go
> where took two corrections.** UM-NATOS-030 added all three DMA resets rather
> than only the outbound channel: `OUT_RST`, `AHBM_FIFO_RST` and `AHBM_RST`. The
> reason is a property of this driver rather than of the hardware —
> **it alternates transports inside one window.** `set_window()` sends its
> command and parameter bytes through the W registers on the FIFO path, then the
> pixels go by DMA, and the two share the peripheral's AHB master FIFO. Switching
> between them without clearing it leaves the engine reading from a buffer the
> CPU path was using.
>
> The correction after that one is the interesting half. Resetting the *outbound
> channel* per transfer turned out to be actively harmful — the first transfer
> completes and the second never signals EOF, because the reset tears down the
> state that produces the signal — so the channel reset belongs at init, and only
> the shared AHB FIFO is cleared per transfer. That was invisible for as long as
> it was, because the constant naming the outbound reset was itself wrong by one
> bit and the reset had never actually happened. **Two wrongs had been cancelling
> out**, which is the failure mode that makes a correct fix look like a
> regression.
...
>
> The two sessions themselves are Chapters 28b and 28c: the eleven theories and
> how each died, the instruments built to settle it, and the two that finally
> did.
...
out of the machine and look at it — which is what `fbdump` finally did (Ch. 28c
§28c.4, and Ch. 28 §28.12 for what it got wrong on its first run).

The full account of how this was found — and of the day spent not finding it —
is Chapters 28b and 28c. Three things from it belong here, in the display
chapter, because they are facts about this driver rather than about method:

- **The heal and the spurious timeout are the same event.** The view came good
  ten to sixty seconds after boot because a preempted display task tripped the
  wall-clock bound, which permanently disabled DMA and moved every transfer onto
  the FIFO path — where this defect does not exist.
- **The threshold in `spi_tx()` decides which consumers could see it.**
  `gfxrogue` fills 180 px, 360 bytes a row, over the 64-byte line: it used DMA
  and provoked the timeout early, which is why it "repaired" the view. The
  `draw` application fills 20 px, 40 bytes a row: entirely FIFO, and it never
  repaired anything.
- **With DMA genuinely working, the blit is 31.4 ms** against a ~31 ms
  theoretical floor at 40 MHz. The transport performance this driver was
  designed for had been in the tree the whole time, behind one bit.
...
days behind a calibration that could only ever give one answer.
