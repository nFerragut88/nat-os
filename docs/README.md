...
| [UM-NATOS-040](UM-NATOS-040-the-heap-that-wasnt-there.md) | The Heap That Wasn't There | a 2 KB static-DRAM overrun pushed _bss_end past _heap_end, the heap range inverted silently, heap_init() returned zero bytes and boot died several self-tests deep in LoadProhibited @ 0 with no link-time warning; APPCPU_CAP_MAX trimmed 8192 -> 6144 and a strict linker ASSERT added so the class becomes a build error; also the X20 verdict -- on this silicon a windowed callee may legally allocate its own window bit and return via retw.n, clearing the vendor-call path |
| [UM-NATOS-041](UM-NATOS-041-the-callback-that-cannot-block.md) | The Callback That Cannot Block | the failure 039 exposed, characterised and then FIXED: esp_wifi_init_internal died with IllegalInstruction at w2c_call2's retw because a0 was no longer a return address, and the word it is loaded from was being overwritten while the task blocked; all four w2c_* bridges saved the windowed a0 at [sp+0], which is the call0 callee's own argument/spill area, so the callee overwrote it exactly as the ABI permits -- moved to a12, which call0 makes the callee preserve; init now executes without faulting for the first time since the vendor stack ran from flash, and blocks instead on a queue only an ISR or timer could fill, neither of which is built; also scopes the callback architecture and records ten instruments that reported something other than what they were trusted for |
| [UM-NATOS-042](UM-NATOS-042-the-shape-of-the-remaining-fault.md) | The Shape of the Remaining Fault | diagnostic stock-take after 041's fix: the address space and three-window blob split, the call0/windowed boundary and its eight bridges, the task model, and the three-layer exclusion design, written so the work can be picked up without the step log; then the remaining fault -- one word of one frame's base save area holding a blob global where a return encoding belongs, with eleven candidates positively eliminated by measurement (restore, terminator, bridges, a12, spill, call widths, switch-frame overlap, shifted layout, bit(base), the blob's use of our critical API, stack pointers); records that excvaddr is 0x170 in every run and a7 therefore equals OSI_FOREVER_CAP exactly, marked explicitly as an untested correlation with the one-constant experiment that would settle it; and catalogues the method failures that cost fourteen retired accounts |
