...
| [UM-NATOS-041](UM-NATOS-041-the-callback-that-cannot-block.md) | The Callback That Cannot Block | the failure 039 exposed and left unfixed, characterised: esp_wifi_init_internal reaches OSI entry 29 (_queue_recv) and dies with IllegalInstruction at w2c_call2's retw because a0 is no longer a return address; an eight-entry ring at the retw shows WINDOWSTART dropping 0x2aaa -> 0x2000 across one switch, seven live frames disclaimed down to one by nat-os's own restore rule, their registers reused before the task resumes; the pin is released by design on the blocking path, the spill did not reduce to one frame, and the interrupt mask is advisory -- the gap where all three protections stop applying at once; also scopes the callback architecture: the ABI crossing is solved (w2c bridges, 118-entry OSI table), ISR-context and event callbacks are unbuilt |
