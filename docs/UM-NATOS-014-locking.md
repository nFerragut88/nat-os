...
nat-os had no way to make anything mutually exclusive. UM-NATOS-010 §8 recorded
the heap as task-context-only with no locking; UM-NATOS-013 §7 recorded console
...
- **`TASK_BLOCKED`.** The scheduler skips it. `TASK_READY` was previously the
  only live state (UM-NATOS-010 §8).
...
(UM-NATOS-009 §11), so the hazard became real the same day.
