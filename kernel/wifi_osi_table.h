/* nat-os -- WiFi OS adapter table. See wifi_osi_table.c (generated). */
#ifndef NATOS_WIFI_OSI_TABLE_H
#define NATOS_WIFI_OSI_TABLE_H
...
/* The precompiled blob was built for ESP32. The struct below is #if'd per
 * target -- get this wrong and the table has the wrong SHAPE, which the blob
 * cannot detect and which calls the wrong function through the right slot. */
 ...
 * retyped. Regenerate wifi_osi_table.c if the IDF version moves. */
typedef struct {
    int32_t _version;
...
const void *wifi_osi_table(void);
uint32_t    wifi_osi_entries(void);
uint16_t    wifi_osi_calls(uint32_t i);
