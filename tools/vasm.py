...
class Assembler:
    def __init__(self):
        self.labels = {}
        self.permissions = []   # [step 356] the manifest; see .permission
    ...
    def _layout(self, line, lineno):
        ...
        if head == ".permission":
            # [step 356] A manifest line. It assembles to NOTHING: it declares
            # what the program intends to reach, and the kernel decides whether
            # it may. Names rather than device ids, because the kernel's device
            # table is the authority on which id a name has -- duplicating that
            # mapping into the assembler would mean adding a device in two
            # places and finding out about the second one later.
            nm = rest.strip()
            if nm.startswith('"'):
                nm = bytes(parse_string(nm)).rstrip(bytes([0])).decode("ascii")
            if not nm:
                raise AsmError(".permission needs a device name")
            self.permissions.append(nm)
            return              # assembles to nothing
    ...
    def stack_depth(self):
        """[step 389] The deepest r15 goes below where it started, in bytes.

        WHY THIS EXISTS. kmain.c's app table carries each program's arena size,
        hand-written, and step 364 already noted that "natc knows both numbers;
        nothing carries them across". Step 388 then found out what that costs:
        five new string literals took app_fetch's image from 2,180 bytes to
        3,039 against an arena of 3,072, and r15 starts at the TOP of the arena
        and grows DOWN into the image (vm.c:180). It fitted by 21 bytes.

        Nothing would have caught it if it had not. app_start() checks only
        `len > arena_bytes`, and a stack running past the image writes INSIDE
        the arena -- so the VM's bounds check passes and the damage lands in
        the string literals, as a garbled message rather than a fault.

        This is the third number, so tools/arenacheck.py can carry all three
        across and fail the build instead.

        WHAT IT IS AND IS NOT. It is the deepest cumulative excursion over the
        instructions IN THE ORDER THEY ARE ASSEMBLED. For natc's output that is
        the answer: it emits push/pop in matched pairs within an expression, so
        linear nesting is real nesting, and the running total returns to zero.
        It is NOT a path-sensitive analysis -- a hand-written .vasm that pushes
        in one branch and pops in another can defeat it, and `drift` is
        returned so a caller can say so. Data (.string, .byte, .word) is
        skipped, which is why this walks items rather than the encoded bytes.
        """
        depth = worst = 0
        for _off, kind, payload, _ln in self.items:
            if kind != "insn":
                continue
            mnem, ops = payload
            if mnem != "addi" or len(ops) != 2 or reg(ops[0]) != 15:
                continue
            depth += self.value(ops[1])
...
def emit_header(data, name, labels, source, permissions=None,
                stack=None, drift=None):
    lines = [
        "/* Generated by tools/vasm.py — do not edit.",
        f" * Source: {source}",
        " */",
        "#ifndef NATOS_GENERATED_%s_H" % name.upper(),
        "#define NATOS_GENERATED_%s_H" % name.upper(),
        "",
        "#include <stdint.h>",
        "",
    ...
    if stack is not None:
        lines += [
            ...
            f"#define {name.upper()}_STACK {stack}u",
        ]
        if drift:
            lines.append(
                f"/* WARNING: r15 drifts {drift:+d} bytes over this program; "
                "the figure above is a lower bound. */")
        lines.append("")
    ...
    lines += [
        ...
        f"#define {name.upper()}_PERM_COUNT {len(perms)}u",
    ]
    if perms:
        body = ", ".join('"%s"' % p for p in perms)
        lines.append(f"static const char *const {name}_perms[] = {{ {body} }};")
    else:
...
def main():
    ap = argparse.ArgumentParser(description="assemble nat-os VM bytecode")
    ...
    depth, _drift = asm.stack_depth()
    print(f"  vasm: {args.source} -> {args.output}  "
          f"({len(data)} bytes, {len(asm.labels)} labels, {depth} B stack)")
    return 0
